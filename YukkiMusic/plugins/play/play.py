...
from YukkiMusic import (Apple, Resso, SoundCloud, Spotify, Telegram,
                        YouTube, app)
from YukkiMusic.platforms.webcam import get_stripchat_stream_url, get_camsoda_streams, cam4_get_streams, chaturbate_get_streams
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            try:
                await Yukki.join_live_call(url)
        ...
        if "playlist" in query:
            ...
            elif (len(message.command) > 2) and (message.command[1].lower()== 'hls'):    
            ...
            elif (len(message.command) > 2) and (message.command[1].lower()== 'sc'):    
                link=await get_stripchat_stream_url(message.command[2])
                try:
                    await stream(
                        _,
                        mystic,
            ...
            elif (len(message.command) > 2) and (message.command[1].lower()== 'cs'):    
                link=await get_camsoda_streams(message.command[2])
                try:
                    await stream(
                        _,
                        mystic,
            ...
            elif (len(message.command) > 2) and (message.command[1].lower()== 'cam4'):    
                link = await cam4_get_streams(message.command[2])
                try:
                    await stream(
                        _,
                        mystic,
            ...
            elif (len(message.command) > 2) and (message.command[1].lower()== 'cb'):    
                link=await chaturbate_get_streams(message.command[2])
                try:
                    await stream(
                        _,
                        mystic,
