...
from pyrogram.types import (InlineKeyboardButton,
                            InlineKeyboardMarkup, Message,ReplyKeyboardMarkup,WebAppInfo,KeyboardButton)
...
@app.on_message(
    filters.command(get_command("STRIPCHAT"))
    & filters.group
    & ~BANNED_USERS
)

async def sc(client, message: Message):
    if len(message.text.split()) > 1:
        name = message.text.split(None, 1)[1]
        async with aiohttp.request('GET',
            f'https://stripchat.com/api/front/v2/models/username/{name}/cam') as resp:
            assert resp.status == 200
            data=await resp.text()
            print(data)
            server = f"https://b-{data['cam']['viewServers']['flashphoner-hls']}.doppiocdn.com/hls/{data['cam']['streamName']}/master_{data['cam']['streamName']}.m3u8"

            server0 = f"https://b-{data['cam']['viewServers']['flashphoner-hls']}.doppiocdn.com/hls/{data['cam']['streamName']}/{data['cam']['streamName']}.m3u8"
            #.format(data["cam"]["viewServers"]["flashphoner-hls"],data["cam"]["streamName"])

            status=f"Stream status: {data['user']['user']['status']}"
            #.format(data["user"]["user"]["status"])
            #await message.reply_text(
            print(f"{status}\n {server}\n {server0}")
